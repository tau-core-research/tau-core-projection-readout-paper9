...
\subsection{Source-arrow and transport boundary}

Source-frozen remains weaker than source-derived. In particular, neither a
projection kernel nor an endpoint residual supplies the smooth source arrows
needed for physical observer--source transport. The finite root-action
groupoid in the current minimal source packet has zero continuous tangent.
The present corpus therefore admits only two coefficient-free conditional
mechanisms for non-scalar smooth transport: holonomy of an independently
sourced full body connection, or Kato transport
\(\mathcal A_K=[dP_O,P_O]\) of a varying, constant-rank, source-owned access
projector. A scalar \(U(1)\) phase may contribute to clock/phase readout but
does not fix normalized non-scalar optical orientation. These statements
sharpen the projection-induction gate; they do not promote any candidate
kernel scored in this paper to a physical parent connection.
...
galaxy morphology, a Tau morphological body, observer/path projection, and
...
We call the deeper theory-side object the \emph{morphological body}
\(M_\tau\).  This is
...
In this terminology, the internal morphological body is the common
...
Tau Core motivation, other projections of the same internal morphological body
...
readout/projection channels of the deeper Tau morphological body: visible
...
\subsubsection{Full-4D completion of the scalar clock shell}

The scalar factor in Eq.~\eqref{eq:time-readout-shell} is not the general
observer-specific metric readout.  On a source-frozen observer chart the full
coframe descent has ADM form
\begin{equation}
 ds_O^2=-N_O^2d\chi^2
 +q_{ij}^{(O)}(dx^i+\beta_O^i d\chi)
                 (dx^j+\beta_O^j d\chi).
 \label{eq:full-4d-adm-readout}
\end{equation}
For an equatorial circular path with \(\Omega=d\phi/d\chi\), the local
Eulerian tangential speed is
\begin{equation}
 \boxed{
 v_{O,{\rm local}}^2(R)
 =
 {q_{\phi\phi}^{(O)}(R)\over N_O^2(R)}
 \left[\Omega(R)+\beta_O^\phi(R)\right]^2 .}
 \label{eq:full-4d-local-speed}
\end{equation}
The spectroscopic endpoint additionally depends on the null transfer.  For
photon tangent \(k\) and emitter/observer four-velocities
\(u_{\rm em},u_{\rm obs}\),
\begin{equation}
 1+z_O={[-u_{\rm em}\!\cdot k]_{\rm em}
        \over[-u_{\rm obs}\!\cdot k]_{\rm obs}},
 \qquad
 V_{\rm obs}^\tau
 =\mathcal R_{\rm spec}
  [g_O^\tau,u_{\rm em},u_{\rm obs},k,\mathcal I],
 \label{eq:full-4d-spectral-readout}
\end{equation}
where \(\mathcal I\) is the frozen inclination, line-profile, beam and
radiative-transfer ledger.

The complete scoring object is likewise metric-level rather than clock-only.
For source-frozen visible, mixed and unresolved Hessian blocks, define
\[
 E_K=(K_{HH}-K_{\rm std})-CK_{VV}^{-1}C^\dagger.
\]
Only its coframe-pushed image may be sent through the spectral, clock, spatial
or photon terminals.  A nonzero mixed block does not suffice, because a
graph-normal auxiliary completion has \(C\neq0\) and \(E_K=0\).  Joint
terminal scoring must quotient exact duplicates and retain the full
cross-terminal covariance.  The scalar \(\Xi_t\) diagnostics in this paper do
not reconstruct this object and remain unchanged.

Equation~\eqref{eq:time-readout-shell} is recovered only after the spatial
metric, shift and photon-transfer prescription are held fixed and the
remaining mismatch is compressed into one lapse-like scalar factor.  It is
therefore a restricted ablation of a full coframe endpoint, not an exhaustive
test of Tau 4D distortion.  None of the existing scalar-clock replays is
recomputed by this recognition.  A full-coframe replay becomes admissible only
after one source-derived \(D_O(R)\) fixes \(N_O\), \(\beta_O^i\),
\(q_{ij}^{(O)}\), and the null-transfer map before endpoint access, with
\(D_O=I\) retained as the exact GR/null control.

The same common packet now gives an exact conditional shift-only row,
\begin{equation}
 q_\tau
 =-\omega_{uv}P_\beta\mathcal L_O^{-1}B_{OS}^{*}W_\rho
   (D_BCx+D_Aa).
 \label{eq:oriented-shift-row}
\end{equation}
If this dimensionless row physically occupies the standard photon-rapidity
terminal, the signed velocity increment is
\begin{equation}
 h_\tau
 =c\,{(1-\beta_{\rm dyn}^{2})\tanh q_\tau
       \over 1+\beta_{\rm dyn}\tanh q_\tau}.
 \label{eq:oriented-rapidity-velocity}
\end{equation}
Equations~\eqref{eq:oriented-shift-row}--\eqref{eq:oriented-rapidity-velocity}
contain no independently adjustable terminal gain, but they remain
conditional on a source-owned nonzero incidence, orientation, calibration and
terminal occupation.  They are one shift ablation, not the full coframe
response and not a derivation of the scalar \(\Xi_t\) shell.

There is also an exact observer-resolution boundary.  A single scalar
rotation record generally cannot distinguish the body contribution \(D_BCx\)
from the irreducible observer--source contribution \(D_Aa\).  A stack of
terminals permits unique linear attribution only when their visible body and
access images have trivial intersection.  After finite stable quantization,
the complete joint output law must refine the same body/access pairs; equality
of marginal or duplicated readouts is insufficient.  Paper~9 freezes neither
that joint law nor a galactic \(Q_{OS}\), so the new row does not authorize
re-estimation of any existing clock/projection score.
...
vertical/outer-warp window.  A separate exact-transfer readiness audit now
carries a source-side \(q_{\rm warp}\) interval and \(\epsilon_{\rm cross}\)
bound into formula-freeze preparation, but this is not an endpoint score and
still requires a frozen exact-transfer manifest.}
...
The current Paper~2/Paper~9 kernel audit separates four roles.  First,
morphology-only kernels are accepted as the inherited one-dimensional preflight
baseline from Paper~8.  Second, projection-enriched mixed kernels are allowed
only when their source tokens, visibility weights, amplitude/carrier rule, and
non-overlap ledger are frozen before endpoint scoring.  Third, \(\Theta_{\rm
morph}\) history/phase terms are morphology-state corrections, not clock
corrections.  Fourth, \(\Xi_t\) or \(\Xi_{\rm eff}\) terms are clock/readout
interval controls and require independent clock evidence; they cannot reuse
warp, flare, vertical-overlay, or H~I asymmetry inputs already consumed by an
active morphology/projection kernel.  The practical consequence is visible in
the case studies: NGC4088 keeps the accepted warp/history route with
\(\Xi_{\rm eff}=1\), NGC4013 rejects the extra \(\Xi_t\) overlay as
double-counting, NGC5907 is treated as projection-saturated at the current
proxy level, and UGC12506 remains a stress object rather than an endpoint
validation case.  Thus the projection layer is not a universal improvement
term; it is an audited source-frozen kernel family.
...
NGC7331 has advanced from missing exact-transfer measurements to a carried
source-side \(q_{\rm warp}\) interval and \(\epsilon_{\rm cross}\) bound, but it
still needs a frozen exact-transfer manifest before refined replay.  NGC5907 is
projection-saturated at the current proxy level, and NGC4183 remains the
weak/null projection control.
...
The current finite laboratory is a reproducible implementation, not a new Nature-selection theorem. The finite observer response is solved only after the body and Hessian are frozen. Its access matrix, susceptibility, noise and stable cell partition remain explicit inputs. A historical joint instrument calibration constrains that supplied adapter, not the physical seed or universal terminal decomposition. The frozen-load transfer must not be confused with the total derivative through body stabilization.
...
The current observer audit separates a conditional forward construction from physical source selection. A frozen body, occupied observer incidence, preparation, complete instrument and stable resolution can determine records without selecting those inputs from Nature. Equal effects do not fix postmeasurement states, and symmetry within a chosen interaction family does not exclude other symmetric couplings. For galactic inference, these observer constructions do not derive a rotation-curve correction or identify a measured residual as a parent effect. Existing endpoint freezes and scores are unchanged.
\subsection{The joint source-to-record interface}
For a supplied occupied context $c$, let $\varrho_c$ be a positive normalized
joint system--detector state, $U_c$ the recovered interaction, and
$L_{r\alpha}$ a complete detector instrument including failure outcomes.
A stable partition $Q_{OS}$ determines records. Put
\[
 K_{r\alpha}=(I\otimes L_{r\alpha})U_c,\qquad
 \mathcal J_j(\varrho_c)=
 \sum_{r:Q_{OS}(r)=j,\alpha}K_{r\alpha}\varrho_c K_{r\alpha}^\dagger.
\]
Then $p_j=\operatorname{Tr}\mathcal J_j(\varrho_c)$ is nonnegative and
normalized. This imports the already conditional quantum representation;
it does not derive Born probabilities from a real response functional.
For correlated preparations no context-independent map of the system
marginal alone follows. Coarse records require summing unnormalized maps
before conditioning. Effects determine probabilities but not the complete
postmeasurement state. Classical and block-classical/quantum receivers are
specializations, not evidence that quantum and gravitational laws coincide.

\paragraph{Source restrictions are stronger than shared ancestry.}
In the existing single-copy real $S_4$ standard representation, a positive
seed-direction-independent link commuting with the full group is $kI$.
Family covariance alone also permits $kI+\alpha nn^T$. The conditional
body-metric loading specialization therefore retains both link and loading
premises; neither a common seed nor a numerical ratio coincidence establishes
their physical occupation. The old minimal base is not tasked with fixing
every physical apparatus coefficient. Concrete selection belongs to the
enriched source, stabilized body and occupied observer incidence.

\paragraph{External evidence and its limits.}
The QILGST public count archive supports operational instrument reconstruction,
but the current audit does not identify a complete independent pulse/IQ/quantizer
calibration packet. Conventional apparatus models remain viable; no unique
Tau residual is extracted. The acoustic quantum eraser of Bienfait et al.,
\emph{Physical Review X} \textbf{10}, 021055 (2020),
\url{https://doi.org/10.1103/PhysRevX.10.021055}, is a physical precedent
for correlation-dependent access, not a realization of our finite observer
Hamiltonian. Its supporting numerical data are stated to be available on request;
the public supplement distinguishes the short delayed-choice readout from
the longer calibration pulse. No replay or new empirical validation is claimed
for that experiment. The programmable-photonic receiver comparison likewise
supports the mechanism class without identifying a Tau-only source law.
...
lowers RMSE.
The reproducibility package therefore includes a paper-level
\texttt{KernelRecord\_tau\^{}S} status audit.  It classifies the projection, time/clock,
history, path/environment, and closure-control lanes as diagnostic/control,
source-proxy, caveated endpoint-active, or blocked.  The audit's claim is
organizational rather than physical: Paper 9 establishes a source-support and
non-double-counting discipline for projection-enriched kernels, not population
validation of a final path-aware Tau Core readout law.

The full-4D completion sharpens this boundary without changing the numerical
record.  The reported \(\Xi_t\) curves remain scalar diagnostic/control
ablations.  No source-derived nonzero \(D_O(R)\) is supplied here, so this
paper neither scores nor detects a non-Einsteinian Tau metric distortion.
